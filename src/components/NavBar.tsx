...
import LocalSwitch from './LocaleSwitcher'
...
const sections = [
	...
	{ id: 'the-aqida', title: 'The Aqida' },
	{ id: 'key', title: 'Unicode Character Key' },
]
						...
						<Popover.Panel className="absolute inset-x-0 top-0 bg-white/95 py-3.5 shadow-sm [@supports(backdrop-filter:blur(0))]:bg-white/80 [@supports(backdrop-filter:blur(0))]:backdrop-blur">
							{sections.map((section, sectionIndex) => (
								...
								>
									<span
										aria-hidden="true"
										className="font-mono text-sm text-green-900"
									>
							...
							<a
								...
								className={clsx(
									...
									sectionIndex === activeIndex
										? 'border-green-900 bg-green-100 text-green-900 before:text-green-900'
										: 'border-transparent before:text-slate-500 hover:bg-green-200 hover:before:text-slate-900',
								)}
							>
								{section.title}
							</a>
